...
def test_init(workspace_environment):
    ...
    workspace.init()
    assert (
        Path(workspace_environment) / ".terraform"
    ).is_dir(), "Workspace has initialized."
...
def test_plan(workspace):
    ...
    assert isinstance(
        plan, TerraformPlan
    ), "Terraform plan returned on successfull plan."
...
def test_apply_interaction(workspace):
    ...
    ), "Terraform apply failed when interaction is required."


def test_apply_auto_approve(workspace):
    results = workspace.apply(
        auto_approve=True, error_function=print, output_function=print
    )
